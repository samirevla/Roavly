...
export async function POST(request: Request) {
  ...
  if (!user) return Response.json({ error: "Sign in to choose Waymark+." }, { status: 401 });
  ...
  if (!config.stripeSecretKey || !priceId) return Response.json({ error: "Waymark+ billing is not configured yet." }, { status: 503 });
  const origin = new URL(request.url).origin;
  const checkout = await createStripeCheckout({
    mode: "subscription",
    customerEmail: user.email,
    successUrl: `${origin}/?checkout=success&surface=waymark-plus`,
    cancelUrl: `${origin}/?checkout=cancelled&surface=waymark-plus`,
    metadata: { kind: "roavly_plus", user_email: user.email, plan },
    priceId,
  });
  if (!checkout?.url) return Response.json({ error: "Waymark+ billing is not configured yet." }, { status: 503 });
